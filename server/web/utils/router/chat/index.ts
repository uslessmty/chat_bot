...
const getSystemPrompt = (avatar: string) => {
    return `${avatar}
...
router.post(`${PREFIX}/dialog`, verify, async (ctx) => {
    try {
        ...
        const [historys] = await db.query<QueryResult & {
            length?: number
          }>('SELECT * FROM chats WHERE window_id = ? AND user_id = ? ORDER BY create_time DESC LIMIT 10;', [windowId, userId]);
        ...
        await db.query('INSERT INTO chats (window_id, user_id, user_message, avatar_response, create_time, modify_time) VALUES (?, ?, ?, ?, ?, ?)', [windowId, userId, message, content, currentTime, currentTime]);
